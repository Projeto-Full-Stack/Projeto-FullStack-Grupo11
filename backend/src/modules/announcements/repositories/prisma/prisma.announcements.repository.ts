...
export class AnnouncementPrismaRepository implements AnnouncementRepository {
  ...
  async findAll(): Promise<Announcement[]> {
    const announcements = await this.prisma.announcement.findMany({
      include: { image: true, comments: { include: { author: { select: { id: true, name: true, description: true } } } }, user: true },
    });
  ...
  async findOne(id: string): Promise<Announcement> {
    const announcement = await this.prisma.announcement.findUnique({
      where: { id },
      include: { image: true, user: true, comments: { include: { author: { select: { id: true, name: true, description: true } } } } },
    });
